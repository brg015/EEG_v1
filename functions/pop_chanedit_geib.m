...
function [chansout, chaninfo, urchans, com] = pop_chanedit_geib(chans, orichaninfo, varargin);
...
if ~isempty(fig)
    ...
    if dataset_input,
        ...
        % Geib fix
        %-----------------------------------------------------------------%
        % This code is garbage that dies when your template file has more
        % information than your EEG file, so lets fix this nonsense
        for ii=1:length(EEG.chanlocs)
            Chan_ind=find(strcmp({chans.labels},EEG.chanlocs(ii).labels));
            if isempty(Chan_ind)
                % Says something ain't named right
                % Truncate the Ch prefix, then lookup again, make special
                % cases for LVEOG and Cz that occasionaly have different
                % names
                % -> LM and RM too, damn this is a mess
                Lookup=EEG.chanlocs(ii).labels(3:end);
                Chan_ind=find(strcmp({chans.labels},Lookup));
                if ~isempty(Chan_ind)
                    % Do nothing, we're good
                elseif (isempty(Chan_ind) && ~(strcmp(Lookup,'4') || strcmp(Lookup,'31') ...,
                        || strcmp(Lookup,'33') || strcmp(Lookup,'64')))
                    disp(['Missing info on: ' EEG.chanlocs(ii).labels]);
                    return;
                elseif strcmp(Lookup,'4')
                    Lookup='Cz';
                    Chan_ind=find(strcmp({chans.labels},Lookup));
                    if isempty(Chan_ind) 
                        disp(['Missing info on: ' EEG.chanlocs(ii).labels]);
            ...
            % Overwrite EEG info with chans info
            EEG(1).chanlocs(ii) = chans(Chan_ind);
        end
        EEG(1).chaninfo = chaninfo;
