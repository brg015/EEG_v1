...
cfg_freq.method     = 'mtmconvol';
cfg_freq.taper = 'hanning';
cfg_freq.foi =  logspace(log10(4),log10(16.2911),24);
cfg_freq.toi        = -1:0.05:1.7;
cfg_freq.t_ftimwin = [3./cfg_freq.foi(cfg_freq.foi<=3) ...
    4./cfg_freq.foi(cfg_freq.foi> 3 & cfg_freq.foi<=7) ...
    5./cfg_freq.foi(cfg_freq.foi> 7 & cfg_freq.foi<=14) ...
...
cfg_freq.pad        = 'maxperlen';
cfg_freq.type       = 'coherence';
suffix='_ENS_COH_v4';

for iSubj=1:length(RUN.dir.subjects)
    if RUN.dir.plot(iSubj)==0, continue; end
    if strcmp(cfg_freq.output,'fourier')
        save_file=fullfile(RUN.dir.pro,[RUN.dir.subjects{iSubj} '_ITC' suffix '.mat']);
    else
        save_file=fullfile(RUN.dir.pro,[RUN.dir.subjects{iSubj} '_dB' suffix '.mat']);
    end
    if ~exist(save_file,'file')
        clear save_file; % It's reset later
        % Load in data
...
%         csd_data=ft_scalpcurrentdensity([],data);
        ...
        freqlock=ft_freqanalysis(cfg_freq, data); clear data;
        
        if strcmp(cfg_freq.output,'fourier')
            ITC = ft_ITC(booleans,freqlock);
            save_file=fullfile(RUN.dir.pro,[RUN.dir.subjects{iSubj} '_ITC' suffix '.mat']);
            save(save_file,'ITC'); clear ITC freqlock;
        end
        
        if strcmp(cfg_freq.output,'pow')
            ...
            Fz=ft_freqbaseline_custom(cfg_f,freqlock);

            cfg_f.baselinetype='logpower';
            cfg_f.inc_trials=~logical(data.reject | data.lat_rej30);
            cfg_f.baseline=[-.5 .2]; % Not used to z-transform
            dB=ft_freqbaseline_custom(cfg_f,freqlock);
...
%=========================================================================%
if strcmp(cfg_freq.output,'fourier')
    if ~exist(fullfile(RUN.dir.sav,['GA_N' num2str(N) '_freq_ENS_ITC.mat']),'file')
        c=1;
        for iSubj=1:length(RUN.dir.subjects)
            if RUN.dir.plot(iSubj)==0, continue; end
            save_file=fullfile(RUN.dir.pro,[RUN.dir.subjects{iSubj} '_ITC' suffix '.mat']);
            ...
            for jj=1:length(fn)
                GA.(fn{jj})(c,:,:,:)=ITC.itpc.(fn{jj});
            end
            c=c+1;
        end
    end
    save(fullfile(RUN.dir.sav,['GA_N' num2str(N) '_ITC' suffix '.mat']),'GA');
end
